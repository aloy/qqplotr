if (!requireNamespace("shiny", quietly = TRUE)) {
  stop(
    "'shiny' package is needed for the Shiny example app to work properly. Please install it.",
    call. = FALSE
  )
} else {
  require(shiny)
...
shinyServer(
  function(input, output, session) {
    ...
    observeEvent(input$optDistribution, {
      ...
      if (
        input$optDistribution %in%
          c("cauchy", "chisq", "exp", "f", "lnorm", "norm", "t")
      ) {
        updateSliderInput(
          session,
          inputId = "sliderPos1",
    ...
    output$ggPlot <- renderPlot({
      gg <- ggplot(
        data = get(input$inputData),
        mapping = aes_string(sample = input$inputVar)
      )
